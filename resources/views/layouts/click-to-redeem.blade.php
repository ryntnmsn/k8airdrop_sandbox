...
@if($promos->codes->count() != 0)
    @foreach($promos->codes->random(1) as $code)
        ...
        <div id="code-container" class="promo-code-container" style="display:none"><!--PROMOCODE-->
            {{__('Promo code')}}
            <h2 id="code">{{$code->name}}</h2>
            <form action="https://playk8.io/member/redemption-code" method="get" target="_blank">
