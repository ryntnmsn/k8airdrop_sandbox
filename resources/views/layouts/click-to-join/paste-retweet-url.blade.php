<form action="/promo/{{$promos->slug}}/{{$promos->url_id}}/store_register" method="post" class="roboto" enctype="multipart/form-data">
   ...
   <section class="mt-12 text-blue-100 bg-gray-900 p-7 rounded-xl">
        ...
        <div class="mb-4">
            <div class="mb-2">
                <label for="paste_retweet_url">{{__('Paste Retweet URL')}} <span class="text-xs">({{ __('Proof of Participation') }}) </span></label>
            </div>
            <div>
                <input class="w-full rounded-md bg-gray-800 border border-gray-700"  type="text" name="paste_retweet_url" id="paste_retweet_url" value="{{old('paste_retweet_url')}}" placeholder="https://twitter.com/k8official_en">
            </div>
        </div>
        
        <div class="mb-4">
            <div class="mb-2">
                <label for="k8_username">{{__('Comments')}} <span class="text-xs">({{ __('Optional') }})</span></label>
            </div>
            <div>
                <textarea class="w-full rounded-md bg-gray-800 border border-gray-700 h-64"  name="comment" id="comment" value="{{old('comment')}}"></textarea>
            </div>
        </div>
